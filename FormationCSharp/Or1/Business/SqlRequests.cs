...
namespace Or.Business
{
    static public class SqlRequests
    {
        ...
        static readonly string queryBéneficairesCarte = "SELECT IdtCpt, PrenomClient, NomClient FROM Bénéficiaire WHERE NumCarte=@Carte";
        static readonly string queryBéneficaire = "SELECT IdtCpt, PrenomClient, NomClient FROM Bénéficiaire WHERE IdtCpt = @IdtCpt";
        static readonly string queryComptes = "SELECT IdtCpt, NumCarte, TypeCompte FROM COMPTE WHERE IdtCpt = @IdtCpt";
        static readonly string queryInsertBénéf = "INSERT INTO \"Bénéficiaire\" (NumCarte, IdtCpt, PrenomClient, NomClient) VALUES (@Carte,@IdtCpt,@Prenom,@Nom)";
        static readonly string queryDeleteBénéf = "DELETE From \"Bénéficiaire\" WHERE IdtCpt=@IdtCpt";
                ...
                using (var command = new SqliteCommand(queryTransacCarte, connection))
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            Operation Operation_type = Tools.TypeTransaction(cptExt, cptDest);
                            string type = Tools.TypeTransacConverter(Operation_type);
                            Transaction trans = new Transaction(idtTransaction, Tools.ConversionDate(horodatage), montant, cptDest, cptExt, type);
                ...
                using (var command = new SqliteCommand(queryTransacCompte, connection))
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            String type = Tools.TypeTransacConverter(Operation);
        ...
        /// <summary>
        /// Liste des dénéficiaires associées à une carte donné
        /// </summary>
        /// <param name="numCarte"></param>
        /// <returns></returns>
        public static List<Bénéficiaire> ListeBénéficiairesAssociesClient(long maCarte)
        {
            List<Bénéficiaire> bénéficiaires = new List<Bénéficiaire>();

            string connectionString = ConstructionConnexionString(fileDb);
            using (var connection = new SqliteConnection(connectionString))
            {
                connection.Open();
                using (var command = new SqliteCommand(queryBéneficairesCarte, connection))
                {
                    command.Parameters.AddWithValue("@Carte", maCarte);
                    using (var reader = command.ExecuteReader())
                    {
                        int idtcpt;
                        string prenom;
                        ...
                        while (reader.Read())
                        {
                            ...
                            Bénéficiaire bénéficiaire = new Bénéficiaire(maCarte, idtcpt, prenom, nom);
                            bénéficiaires.Add(bénéficiaire);
                        }
                    }
                }
        ...
        public static SqliteCommand AjouterBénéficiaire(Bénéficiaire Bénéf)
        {
            string connectionString = ConstructionConnexionString(fileDb);
            using (var connection = new SqliteConnection(connectionString))
            {
                ...
                var insertBénéficiaire = connection.CreateCommand();
                insertBénéficiaire.CommandText = queryInsertBénéf;

                insertBénéficiaire.Parameters.AddWithValue("@Carte", Bénéf.MaCarte);
                insertBénéficiaire.Parameters.AddWithValue("@IdtCpt", Bénéf.IdtCpt);
                insertBénéficiaire.Parameters.AddWithValue("@Prenom", Bénéf.PrenomClient);
                insertBénéficiaire.Parameters.AddWithValue("@Nom", Bénéf.NomClient);

                insertBénéficiaire.ExecuteNonQuery();

                return insertBénéficiaire;
            }
        }
        public static SqliteCommand SupprimerBénéficiaire(int IdtCpt)
        {
            string connectionString = ConstructionConnexionString(fileDb);
            using (var connection = new SqliteConnection(connectionString))
            {
                ...
                var DeleteBénéficiaire = connection.CreateCommand();
                DeleteBénéficiaire.CommandText = queryDeleteBénéf;

                DeleteBénéficiaire.Parameters.AddWithValue("@IdtCpt",IdtCpt);

                DeleteBénéficiaire.ExecuteNonQuery();

                return DeleteBénéficiaire;
            }
        }

        public static MessErreur EstBeneficiairePotentiel(long maCarte, int IdtCpt)
        {
            ...
            {
                connection.Open();
                // chercher si le compte peut etre un compte bénéficiaire
                using (var command = new SqliteCommand(queryComptes, connection))
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            // le compte ajouter appartient déjà à ma carte
                            if (maCarte == reader.GetInt64(1))
                            {
                                b = "Votre compte ne peux pas être ajouter à la liste";
                                messErreur.message = b;
                                messErreur.Condition = false;
                            }
                            else if (reader.GetString(2) == "Livret")
                            {
                                b = "Un compte de type LIVRET ne peut pas être rajouter à la liste";
                                messErreur.message = b;
                                messErreur.Condition = false;
                            }
                            // le compte appartient à une carte étrangère
                            else
                            {
                                messErreur.numero = reader.GetInt64(1);
                                messErreur.Condition = true;
                            }
                        }
                    }
                }
                // cas d'erreur si mon compte est déja bénéficiaire
                using (var command = new SqliteCommand(queryBéneficaire, connection))
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            a = "Le compte est déja bénéficiaire";
                            messErreur.message = a;
                            messErreur.Condition = false;
                        }
                    }
                }
            ...
            // cas où le compte n'existe pas dans la liste des compte
            if (messErreur.Condition == false && a==string.Empty && b== string.Empty)
            {
                messErreur.message = "Le bénéficiaire n'existe pas";
            }
            return messErreur;
        }
